...
var EmailSummarizer = /** @class */ (function () {
    function EmailSummarizer(options) {
        ...
        this.maxMessagesInContext = (_b = options.maxMessagesInContext) !== null && _b !== void 0 ? _b : 20;
            ...
            return __generator(this, function (_b) {
                switch (_b.label) {
                    case 0:
                        ...
                        if (this.debug) {
                            console.log("[EmailSummarizer] Summarizing thread: ".concat(thread.subject, " (").concat(thread.messages.length, " messages, mode: ").concat(mode, ")"));
                        }
                        startTime = Date.now();
                        ...
                        return [4 /*yield*/, this.llmClient.complete(llmMessages, {
                    ...
                    case 1:
                        ...
                        if (this.debug) {
                            console.log("[EmailSummarizer] Generated summary in ".concat(generationTimeMs, "ms (").concat(result.totalTokens, " tokens)"));
                        }
                        return [2 /*return*/, __assign(__assign({}, summary), { mode: mode, participants: thread.participants.map(function (p) { return p.email; }), messageCount: messages.length, tokensUsed: result.totalTokens, generationTimeMs: generationTimeMs })];
                }
            });
        });
    };
            ...
            return __generator(this, function (_b) {
                switch (_b.label) {
                    case 0:
                        mode = (_a = options.mode) !== null && _a !== void 0 ? _a : this.defaultMode;
                        if (this.debug) {
                            console.log("[EmailSummarizer] Summarizing email: ".concat(email.subject, " (mode: ").concat(mode, ")"));
                        }
                        startTime = Date.now();
                    ...
                    case 1:
                        ...
                        return [2 /*return*/, __assign(__assign({}, summary), { mode: mode, participants: __spreadArray([email.from.email], email.to.map(function (t) { return t.email; }), true), messageCount: 1, tokensUsed: result.totalTokens, generationTimeMs: generationTimeMs })];
                }
            });
        });
    };
    ...
    EmailSummarizer.prototype.getSystemPrompt = function (mode) {
        ...
        switch (mode) {
            case 'brief':
                return "".concat(basePrompt, " Provide ultra-concise summaries in 1-2 sentences that capture the core message.");
            case 'detailed':
                return "".concat(basePrompt, " Provide detailed summaries that capture key points, decisions, and context.");
            case 'action-items':
                return "".concat(basePrompt, " Extract and list action items, tasks, and next steps from the conversation. Format as a bulleted list with action, assignee (if mentioned), and deadline (if mentioned).");
            case 'key-points':
                return "".concat(basePrompt, " Extract the key points and important information from the conversation. Format as a bulleted list.");
            default:
                return basePrompt;
        }
    };
    ...
    EmailSummarizer.prototype.buildPrompt = function (thread, messages, mode) {
        ...
        if (this.includeMetadata) {
            ...
            parts.push("Participants: ".concat(thread.participants.map(function (p) { return p.name || p.email; }).join(', ')));
        ...
        messages.forEach(function (msg, index) {
            ...
            var timestamp = _this.includeMetadata ? " (".concat(_this.formatDate(msg.receivedAt), ")") : '';
            parts.push("[".concat(index + 1, "] ").concat(fromName).concat(timestamp, ":"));
        ...
        switch (mode) {
            case 'brief':
                ...
                break;
            case 'detailed':
                parts.push('Provide a detailed summary covering the main points, decisions made, and any important context.');
                break;
            case 'action-items':
                parts.push('Extract all action items, tasks, and next steps. Format each as:\n- Action: [description]\n  Assignee: [person if mentioned]\n  Due: [date if mentioned]');
                break;
            case 'key-points':
    ...
    EmailSummarizer.prototype.buildSingleEmailPrompt = function (email, mode) {
        ...
        if (this.includeMetadata) {
            ...
            parts.push("To: ".concat(email.to.map(function (t) { return t.name || t.email; }).join(', ')));
        ...
        switch (mode) {
            ...
            case 'detailed':
                ...
                break;
            case 'action-items':
                parts.push('Extract all action items and tasks mentioned. Format each as:\n- Action: [description]\n  Assignee: [person if mentioned]\n  Due: [date if mentioned]');
                break;
            case 'key-points':
    ...
    EmailSummarizer.prototype.prepareMessages = function (messages) {
        ...
        if (this.debug) {
            console.log("[EmailSummarizer] Truncating ".concat(messages.length, " messages to ").concat(this.maxMessagesInContext));
        }
