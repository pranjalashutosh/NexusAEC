...
var NarrativeGenerator = /** @class */ (function () {
    function NarrativeGenerator(options) {
        ...
        this.defaultStyle = (_a = options.defaultStyle) !== null && _a !== void 0 ? _a : 'conversational';
    ...
    NarrativeGenerator.prototype.generateBriefing = function (input_1) {
        return __awaiter(this, arguments, void 0, function (input, options) {
            var style, startTime, segments, totalTokens, redFlagCount, opening, sortedClusters, topicClusters, i, cluster, isLast, transition, topic, closing, generationTimeMs, totalSeconds;
            var _a;
            ...
            return __generator(this, function (_b) {
                switch (_b.label) {
                    case 0:
                        ...
                        startTime = Date.now();
                        if (this.debug) {
                            console.log("[NarrativeGenerator] Generating briefing: ".concat(input.clusters.length, " clusters, ").concat(input.redFlagScores.size, " red flags, style: ").concat(style));
                        }
                        segments = [];
                        totalTokens = 0;
                        redFlagCount = 0;
                    ...
                    case 8:
                        ...
                        totalSeconds = segments.reduce(function (sum, seg) { return sum + seg.estimatedSeconds; }, 0);
                        if (this.debug) {
                            console.log("[NarrativeGenerator] Generated ".concat(segments.length, " segments in ").concat(generationTimeMs, "ms (").concat(totalTokens, " tokens, ~").concat(totalSeconds, "s reading time)"));
                        }
                        return [2 /*return*/, {
                                segments: segments,
    ...
    NarrativeGenerator.prototype.generateOpening = function (input, style) {
        return __awaiter(this, void 0, void 0, function () {
            var currentTime, timeOfDay, userName, clusterCount, redFlagCount, systemPrompt, userPrompt, messages, result;
            var _a, _b;
            return __generator(this, function (_c) {
                switch (_c.label) {
                    case 0:
                        ...
                        redFlagCount = Array.from(input.redFlagScores.values()).filter(function (s) { return s.isFlagged; })
                            .length;
                        systemPrompt = this.getStyleSystemPrompt(style);
                        userPrompt = "Generate a brief opening greeting for an email briefing. Context:\n- Time of day: ".concat(timeOfDay, "\n- User name: ").concat(userName || 'executive', "\n- Total topics: ").concat(clusterCount, "\n- Red flags: ").concat(redFlagCount, "\n\nGenerate 1-2 sentences that welcome the user and preview what's ahead. Be natural and concise.");
                        messages = [
                            { role: 'system', content: systemPrompt },
                            { role: 'user', content: userPrompt },
                        ];
                        return [4 /*yield*/, this.llmClient.complete(messages, {
                                temperature: 0.7,
                                maxTokens: 150,
                            })];
                    case 1:
        ...
        return __awaiter(this, void 0, void 0, function () {
            ...
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        systemPrompt = this.getStyleSystemPrompt(style);
                        userPrompt = "Generate a brief closing for an email briefing. Context:\n- Red flags mentioned: ".concat(redFlagCount, "\n- Topics covered: ").concat(Math.min(input.clusters.length, this.maxTopics), "\n\nGenerate 1-2 sentences that wrap up the briefing and prompt for user interaction. Be natural and concise.");
                        messages = [
                            { role: 'system', content: systemPrompt },
                            { role: 'user', content: userPrompt },
                        ];
    ...
    NarrativeGenerator.prototype.generateTopicNarrative = function (cluster, input, style) {
        return __awaiter(this, void 0, void 0, function () {
            var redFlags, summaries, _i, _a, threadId, summary, _b, _c, emailId, summary, systemPrompt, userPrompt, messages, result;
            return __generator(this, function (_d) {
                switch (_d.label) {
                    case 0:
                        redFlags = cluster.emailIds
                            .map(function (id) { return input.redFlagScores.get(id); })
                            .filter(function (score) { return (score === null || score === void 0 ? void 0 : score.isFlagged) === true; });
                        summaries = new Set();
    ...
    NarrativeGenerator.prototype.buildTopicPrompt = function (cluster, summaries, redFlags) {
        ...
        if (redFlags.length > 0) {
            ...
            var urgentReasons = redFlags
                .flatMap(function (flag) { return flag.reasons.map(function (r) { return r.reason; }); })
                .slice(0, 3);
        ...
        parts.push('Generate 2-4 sentences that naturally present this topic, incorporating the key points and urgency signals. Speak directly to the user as if in a conversation.');
        return parts.join('\n');
    };
    /**
    ...
    NarrativeGenerator.prototype.sortClustersByPriority = function (input) {
        return __spreadArray([], input.clusters, true).sort(function (a, b) {
            ...
            var aFlags = a.emailIds.filter(function (id) { var _a; return (_a = input.redFlagScores.get(id)) === null || _a === void 0 ? void 0 : _a.isFlagged; }).length;
            var bFlags = b.emailIds.filter(function (id) { var _a; return (_a = input.redFlagScores.get(id)) === null || _a === void 0 ? void 0 : _a.isFlagged; }).length;
            // Get highest red flag score
            var aMaxScore = Math.max.apply(Math, __spreadArray(__spreadArray([], a.emailIds.map(function (id) { var _a, _b; return (_b = (_a = input.redFlagScores.get(id)) === null || _a === void 0 ? void 0 : _a.score) !== null && _b !== void 0 ? _b : 0; }), false), [0], false));
            var bMaxScore = Math.max.apply(Math, __spreadArray(__spreadArray([], b.emailIds.map(function (id) { var _a, _b; return (_b = (_a = input.redFlagScores.get(id)) === null || _a === void 0 ? void 0 : _a.score) !== null && _b !== void 0 ? _b : 0; }), false), [0], false));
    ...
    NarrativeGenerator.prototype.getStyleSystemPrompt = function (style) {
        var basePrompt = 'You are an executive assistant creating a voice briefing script. Generate natural, spoken language suitable for audio delivery.';
        switch (style) {
            case 'formal':
                return "".concat(basePrompt, " Use formal, professional language. Be respectful and precise.");
            case 'conversational':
                return "".concat(basePrompt, " Use warm, conversational language. Sound like a trusted colleague giving a friendly update.");
            case 'executive':
                return "".concat(basePrompt, " Be concise and direct. Use short sentences. Get to the point quickly.");
            case 'concise':
                return "".concat(basePrompt, " Be extremely brief. Use minimal words while conveying essential information.");
            default:
                return basePrompt;
        }
    };
