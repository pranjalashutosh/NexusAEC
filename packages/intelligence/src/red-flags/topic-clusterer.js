...
var TopicClusterer = /** @class */ (function () {
    ...
    TopicClusterer.prototype.clusterEmails = function (emails) {
        ...
        emails.forEach(function (email) { return emailMap.set(email.id, email); });
        ...
        if (this.options.useThreadIds) {
            ...
            for (var _i = 0, emails_1 = emails; _i < emails_1.length; _i++) {
                ...
                if (email.threadId) {
                    ...
                    (_a = threadGroups.get(email.threadId)) === null || _a === void 0 ? void 0 : _a.add(email.id);
                }
            }
            // Convert thread groups to clusters
            for (var _g = 0, _h = threadGroups.entries(); _g < _h.length; _g++) {
                var _j = _h[_g], threadId = _j[0], emailIds = _j[1];
                if (emailIds.size >= this.options.minClusterSize) {
                    clusters.set("thread-".concat(threadId), emailIds);
                    emailIds.forEach(function (id) { return processedEmails.add(id); });
                }
            }
        }
        // Step 2: Subject-based clustering for remaining emails
        var remainingEmails = emails.filter(function (email) { return !processedEmails.has(email.id); });
            ...
            var _loop_1 = function (i) {
                ...
                for (var j = i + 1; j < subjectList.length; j++) {
                    ...
                    if (similarity >= this_1.options.similarityThreshold) {
                        ...
                        if (group2) {
                            group2.forEach(function (id) { return group1.add(id); });
                            merged.add(subject2);
                        }
                    }
                }
                ...
                if (group1.size >= this_1.options.minClusterSize) {
                    ...
                    group1.forEach(function (id) { return processedEmails.add(id); });
                }
            };
            var this_1 = this;
        ...
        var _loop_2 = function (_key, emailIds) {
            var emailList = Array.from(emailIds)
                .map(function (id) { return emailMap.get(id); })
            ...
            emailList.forEach(function (email) {
                ...
                keywords.forEach(function (kw) { return allKeywords.add(kw); });
            });
            ...
            for (var i = 0; i < emailList.length; i++) {
                for (var j = i + 1; j < emailList.length; j++) {
                    ...
                    var kw1 = extractKeywords("".concat(email1.subject, " ").concat((_d = (_c = email1.body) !== null && _c !== void 0 ? _c : email1.snippet) !== null && _d !== void 0 ? _d : ''));
                    var kw2 = extractKeywords("".concat(email2.subject, " ").concat((_f = (_e = email2.body) !== null && _e !== void 0 ? _e : email2.snippet) !== null && _f !== void 0 ? _f : ''));
            ...
            var threadIds = Array.from(new Set(emailList.map(function (e) { return e.threadId; }).filter(function (id) { return id !== undefined; })));
            finalClusters.push({
        ...
        for (var _l = 0, _m = clusters.entries(); _l < _m.length; _l++) {
            var _o = _m[_l], _key = _o[0], emailIds = _o[1];
            _loop_2(_key, emailIds);
        }
        // Sort clusters by size (descending)
        finalClusters.sort(function (a, b) { return b.size - a.size; });
        // Find unclustered emails
        var unclusteredEmailIds = emails
            .filter(function (email) { return !processedEmails.has(email.id); })
            .map(function (email) { return email.id; });
        return {
    ...
    TopicClusterer.prototype.clusterThreads = function (threads) {
        ...
        var allEmails = threads.flatMap(function (thread) { return thread.messages; });
