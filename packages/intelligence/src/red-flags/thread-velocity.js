...
function detectEscalationLanguage(email) {
    var _a, _b;
    var matches = [];
    var text = "".concat(email.subject, " ").concat((_b = (_a = email.body) !== null && _a !== void 0 ? _a : email.snippet) !== null && _b !== void 0 ? _b : '');
    for (var _i = 0, ESCALATION_PATTERNS_1 = ESCALATION_PATTERNS; _i < ESCALATION_PATTERNS_1.length; _i++) {
...
function calculateReplyFrequency(messages, windowHours, now) {
    ...
    var recentMessages = messages.filter(function (msg) { return msg.receivedAt >= cutoffTime; });
...
var ThreadVelocityDetector = /** @class */ (function () {
    function ThreadVelocityDetector(options) {
        ...
        this.options = {
            highVelocityWindowHours: (_a = options.highVelocityWindowHours) !== null && _a !== void 0 ? _a : 2,
            highVelocityThreshold: (_b = options.highVelocityThreshold) !== null && _b !== void 0 ? _b : 4,
            highVelocityWeight: (_c = options.highVelocityWeight) !== null && _c !== void 0 ? _c : 0.7,
            mediumVelocityWindowHours: (_d = options.mediumVelocityWindowHours) !== null && _d !== void 0 ? _d : 6,
            mediumVelocityThreshold: (_e = options.mediumVelocityThreshold) !== null && _e !== void 0 ? _e : 3,
            mediumVelocityWeight: (_f = options.mediumVelocityWeight) !== null && _f !== void 0 ? _f : 0.5,
            escalationLanguageWeight: (_g = options.escalationLanguageWeight) !== null && _g !== void 0 ? _g : 0.8,
        };
    }
    /**
    ...
    ThreadVelocityDetector.prototype.analyzeEmails = function (emails) {
        ...
        var sortedEmails = __spreadArray([], emails, true).sort(function (a, b) { return a.receivedAt.getTime() - b.receivedAt.getTime(); });
        ...
        var highVelocityCount = calculateReplyFrequency(sortedEmails, this.options.highVelocityWindowHours, now);
        if (highVelocityCount >= this.options.highVelocityThreshold) {
            score += this.options.highVelocityWeight;
            reasons.push({
                type: 'high_velocity',
                description: "".concat(highVelocityCount, " replies in ").concat(this.options.highVelocityWindowHours, " hours"),
                weight: this.options.highVelocityWeight,
            });
        }
        else {
            // Check for medium velocity
            var mediumVelocityCount = calculateReplyFrequency(sortedEmails, this.options.mediumVelocityWindowHours, now);
            if (mediumVelocityCount >= this.options.mediumVelocityThreshold) {
                score += this.options.mediumVelocityWeight;
                reasons.push({
                    type: 'medium_velocity',
                    description: "".concat(mediumVelocityCount, " replies in ").concat(this.options.mediumVelocityWindowHours, " hours"),
        ...
        if (avgTimeBetweenReplies < 15 && emails.length >= 3) {
            ...
            reasons.push({
                ...
                description: "Rapid back-and-forth: avg ".concat(Math.round(avgTimeBetweenReplies), " min between replies"),
                weight: rapidWeight,
            });
        }
        ...
        if (hasEscalationLanguage) {
            ...
            reasons.push({
                ...
                description: "Escalation language detected: \"".concat(uniquePhrases.slice(0, 3).join('", "'), "\""),
